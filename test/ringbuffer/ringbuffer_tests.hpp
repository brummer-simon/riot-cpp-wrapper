...
auto ringbufferTestConstructor(size_t& succeededTests, size_t& failedTests) -> void
{
    // Default Constructor
    riot::Ringbuffer<TestObj, 1> rbuf1;
    rbuf1.addOne(TestObj(1,2,3));
    // Copy-Constructor
    riot::Ringbuffer<TestObj, 1> rbuf2(rbuf1);
    TestObj ret1;
    rbuf2.getOne(ret1);
    if (ret1 != TestObj(1,2,3)) {
        printf("Test '%s' failed.\n", __PRETTY_FUNCTION__);
        printf("!--- Reason: (ret1 != TestObj(1,2,3))\n");
        failedTests += 1;
    }
    // Assignment Operator
    riot::Ringbuffer<TestObj, 1> rbuf3;
    rbuf3 = rbuf1;
    TestObj ret2;
    rbuf3.getOne(ret2);
    if (ret2 != TestObj(1,2,3)) {
        printf("Test '%s' failed.\n", __PRETTY_FUNCTION__);
        printf("!--- Reason: (ret2 != TestObj(1,2,3))\n");
        failedTests += 1;
    }
...
{
    ringbufferTestConstructor(succeededTests, failedTests);
